...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        pplace = self.place
        dist = 0
        while not pplace.is_hive and dist <= self.upper_bound:
            if pplace.bees and self.lower_bound <= dist:
                return random_bee(pplace.bees)
            pplace = pplace.entrance    
            dist += 1 
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if amount >= self.health:
            totttal = amount + self.damage
        for bee in self.place.bees[:]:
            bee.reduce_health(totttal)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        for bee in self.place.bees:
            result[bee] = distance
        while not p.entrance.is_hive:
            distance += 1
            p = p.entrance
            for bee in p.bees:
                result[bee] = distance
            if p.ant:
               result[p.ant] = distance
               if p.ant.is_container:
                result[p.ant.ant_contained] = distance
        return result                
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        result = self.damage - 0.25 * distance - self.insects_shot / 16
        if result > 0:
            return result
        return 0
        # END Problem Optional 2
