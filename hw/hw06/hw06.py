...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        yyears = Mint.present_year - self.year - 50
        if yyears > 0:
            return self.cents + yyears
        else:
            return self.cents    
...
def deep_map_mut(func, lnk):
    ...
    """
    "*** YOUR CODE HERE ***"
    if isinstance(lnk.first,Link) and lnk.first:
...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        b = t.branches[0]
        return is_bst(b) and (bst_max(b).label <= t.label or bst_min(b).label > t.label)
    else:
        b0, b1 = t.branches[0], t.branches[1]
        return is_bst(b0) and is_bst(b1) and bst_max(b0).label <= t.label and bst_min(b1).label > t.label
    """def helper(root,l,r):  另解（我只能理解这个）
