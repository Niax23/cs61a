...
def subseqs(s):
    ...
    if not s:
        ...
        sub = subseqs(s[1:])
        return sub + insert_into_all(s[0],subseqs(s[1:]))
...
class VendingMachine:
    ...
    def restock(self,amount):
        self.stocks += amount
        return f'Current {self.goods} stock: {self.stocks}'    
    ...
    def vend(self):
        ...
        if self.balance < self.price:
            ...
            self.balance = 0
            return f'Here is your {self.goods} and ${t} change.'                   
...
def reverse_other(t):
    ...
    def helper(root,rev):
        if root.is_leaf():
            return
        if rev:
            revv = [b.label for b in root.branches[::-1]]
            for i in range(len(root.branches)):
                root.branches[i].label = revv[i]
