...
def tail_points(opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    assert opponent_score >= 0, 'Must be none-negative.'
    i = opponent_score % 10
...
def perfect_square(i):
    assert i >= 0,'We are talking about real numbers'
    j = 1
    while j * j <= i:
        if j * j == i:
            return True
...
sqrt = invverse(sqquare)
def next_perfect_square(item):
    k = sqrt(item)
    k += 1
    return sqquare(k)
...
def play(strategy0, strategy1, update,
    ...
    while score0 < goal and score1 < goal:
        ...
        if who == 0:
            s = strategy0(score0,score1)
            score0 = update(s,score0,score1,dice)
        else:
            s = strategy1(score1,score0)
            score1 = update(s,score1,score0,dice)
        who = 1 - who
    # END PROBLEM 5
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    max_value,mark,i = 0,0,0
    averagg = make_averaged(roll_dice,total_samples)
    while i < 10:
        i += 1
        kk = averagg(i,dice)
        if kk > max_value:
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    re = square_update(0,score,opponent_score) - score
    if re >= threshold:
        num_rolls = 0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
