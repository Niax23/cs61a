...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def inner(ss):
         t = split(remove_punctuation(lower(ss)))
         for i in topic:
             for j in t:
                if i == j:
                    return True
...
def accuracy(typed, source):
    ...
    if not typed_words and not source_words:
        ...
        return 0.0
         ...
         counnt,total,lenn = 0,len(typed_words),min(len(typed_words),len(source_words))
         for i in range(lenn):
            if typed_words[i] == source_words[i]:
                counnt += 1
         return 100 * counnt / total       
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for i in word_list:
        if i == typed_word:
            return typed_word
        elif diff_function(typed_word,i,limit) < mind:
            mind =  diff_function(typed_word,i,limit)
            target = i
    if mind <= limit:
        return target
    else:
...
def feline_fixes(typed, source, limit):
    ...
    if not typed or not source:
        ...
        elif typed[0] != source[0] and limit == 0:
            return 1
        else:
...
def minimum_mewtations(start, goal, limit):
    ...
    if not start or not goal: 
        return len(start) + len(goal) # Fill in the condition
        # BEGIN
        "*** YOUR CODE HERE ***"
        # END
    # elif start[0] == goal[0]:
    ...
    #     return minimum_mewtations(start, goal, limit)  
        ...
        return 1  # Feel free to remove or add additional cases
        # BEGIN
        "*** YOUR CODE HERE ***"
        # END
    elif start[0] == goal[0]:
...
def report_progress(typed, prompt, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] != prompt[i]:
            d = {'id': user_id,'progress': i / len(prompt)}
            upload(d)
            return i / len(prompt)
    d = {'id': user_id,'progress': len(typed) / len(prompt)}
    upload(d)
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    a = [[bb[i] - bb[i - 1] for i in range(1,len(bb))] for bb in times_per_player]
    return match(words,a)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    wn = len(get_all_times(match))
    re = [[]]* wn
    for i in word_indices:
        max,target = 10000,0
        for j in player_indices:
            if time(match,j,i) < max:
                target = j
                max = time(match,j,i)
        re[target] = re[target]+[get_word(match,i)]        
    return re    
